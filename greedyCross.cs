...
public class Rubik{
	...
	public static void Main(String[] args){
		...
		Console.WriteLine(greedyCross(0,start,lst);
...
class DepthFirst{
    ...
    */
	...
	static public Cube greedyCross(int depth, List<Cube> explored, List<Cube> frontier);
	{
		Cube c;
		if (frontier.Count == 0);
			{c = explored[0];}
			...
			int largestNum = (-1 * depth) - 1;
			Cube tempCube;
			foreach (Cube cube in frontier){				
				if (WhiteCross(cube)- depth)>largestNum)
					{
						largestNum = WhiteCross(cube)- depth;
						c = cube;
					}
			}
		...
		List<Cube> tempList = new List<Cube>();
				...
				greedyCross(depth + 1, explored, frontier)
